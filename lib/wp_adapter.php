...
function get_bloginfo($param)
{
	...
	switch ($param) {
		...
		case 'rdf_url':  //（RDF/RSS 1.0 形式のメインフィードURL）
			return str_replace('cmd=rss', 'cmd=rss&ver=1.0', $qt->getv('rss_link'));
			break;
		case 'rss_url':  //（RSS 0.92 形式のメインフィードURL）
			return $qt->getv('rss_link');
			break;
		case 'rss2_url':  //（RSS 2.0 形式のメインフィードURL）
			return str_replace('cmd=rss', 'cmd=rss&ver=2.0', $qt->getv('rss_link'));
			break;
		case 'atom_url':  //（Atom形式のメインフィードURL）
			return str_replace('cmd=rss', 'cmd=atom', $qt->getv('rss_link'));
			break;
		...
		case 'template_url':  //（使用中テンプレートのURL）（Version 1.5以降）
			return dirname($script) . '/' . TEMPLATEPATH;
			break;
