...
{
	global $script, $vars, $pkwk_dtd;
	$qm = get_qm();

	if (PKWK_READONLY) die_message($qm->m['fmt_err_pkwk_readonly']);

	//戻り値を初期化
	$retval['msg'] = $qm->m['plg_paint']['title'];
	$retval['body'] = '';
	...
	if (
		...
		if (!exist_plugin('attach') or !function_exists('attach_upload')) {
			return array('msg' => $qm->m['plg_paint']['err_plg_attach']);
		}
 ...
 <param name="form1" value="filename={$qm->m['plg_paint']['field_filename']}=!" />
 <param name="form2" value="yourname={$qm->m['plg_paint']['field_name']}" />
 <param name="comment" value="msg={$qm->m['plg_paint']['field_comment']}" />
...
function plugin_paint_convert()
{
	...
	$max = sprintf($qm->m['plg_paint']['max'], PAINT_MAX_WIDTH, PAINT_MAX_HEIGHT);
  ...
  $max
  <input type="submit" value="{$qm->m['plg_paint']['btn_submit']}" />
  </div>
  </form>
EOD;
	return $ret;
...
function paint_insert_ref($filename)
{
	...
	$name = ($name == '') ? $qm->m['fmt_no_name'] : $vars['yourname'];
	...
	if (md5(join('', $postdata_old)) != $vars['digest']) {
		...
		$ret['body'] = $qm->m['plg_paint']['collided'];
	}
