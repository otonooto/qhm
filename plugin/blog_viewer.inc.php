...
// $Id: blog_viewer.inc.php $
//
// Blog Viewer plugin - List pages that calendar/calnedar2 plugin created as a blog entry
// (Based on calendar viewer plugin)
...
function plugin_blog_viewer_convert()
{
	...
	if (preg_match('/[0-9]{4}' . $date_sep . '[0-9]{2}/', $func_args[1])) {
		...
		return $qm->replace('fmt_err_cvt', 'blog_viewer', $qm->m['plg_calendar_viewer']['err_param2']);
	}
	...
	if (isset($viewed[$pagename])) {
		$s_page = h($pagename);
		return $qm->replace('fmt_err_cvt', 'blog_viewer', $qm->replace('plg_blog_viewer.err_already_view', $s_page));
	} else {
	...
	while ($tmp < $limit_page) {
		...
		if (PLUGIN_BLOG_VIEWER_DATE_FORMAT !== FALSE) {
	...
	if ($left_YM != '' || $right_YM != '') {
		...
		$link = $script . '?plugin=blog_viewer&amp;mode=' . $mode .
...
function plugin_blog_viewer_action()
{
	...
	$return_vars_array['body'] = call_user_func_array('plugin_blog_viewer_convert', $args_array);

	//$return_vars_array['msg'] = 'calendar_viewer ' . $vars['page'] . '/' . $page_YM;
	$return_vars_array['msg'] = htmlspecialchars($vars['page']);
