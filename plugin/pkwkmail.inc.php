...
function plugin_pkwkmail_convert()
{
	...
	if (PKWKMAIL_FREEZE_CHECK == '1') {
		if (! is_freeze($vars['page'])) {
			return '<p>' . $qm->m['plg_pkwkmail']['not_use'] . '</p>';
		}
	}
...
function PKWKMAIL_prepare($lines)
{
	...
	foreach ($format_type as $key => $val) {
		...
		switch ($val[0]) {
			case 0:
				$attr[$key] = $qm->m['plg_pkwkmail'][$key];
				break;
			case 1:
				$attr[$key] = convert_html($qm->m['plg_pkwkmail'][$key]);
				break;
			case 2:
				$attr[$key] = $val[1];
				break;
			case 3:
				$attr[$key] = convert_html('[[' . $qm->m['plg_pkwkmail'][$key] . '>' . $val[1] . ']]');
				break;
		}
	}
	...
	if ($attr['admin_adrs'] == 'to@example.com' || $attr['admin_adrs'] == '') {
		die_message(PKWKMAIL_addEditLink($qm->m['plg_pkwkmail']['err_msg_notify_to']));
	}

	foreach (array('admin_adrs', 'admin_reply_to') as $x) {
		if (empty($attr[$x])) continue;
		$rc = PKWKMAIL_MailAddrCheck($attr[$x], $attr['domain_check']);
		if ($rc['err']) die_message(PKWKMAIL_addEditLink(sprintf($qm->m['plg_pkwkmail'][$rc['msg']], $attr[$x]) . ' (' . $x . ')'));
	}
...
function PKWKMAIL_formmaker($attr, $cnfm, $a_page)
{
	...
	foreach ($attr['attr'] as $k => $v) {
		...
		switch ($attr['attr'][$k][1]) {
			...
			case 'checkbox':
				$attr_err_flag = true;
				$attr_error .= "\t" . '<li><strong>' . sprintf($qm->m['plg_pkwkmail']['attr_err_ary'], $attr['attr'][$k][0]) . '</strong></li>' . "\n";
				break;
		}
	}
	...
	$render_title = $render_element = [];

	foreach ($attr['attr'] as $k) {
		//must check - 必須項目チェック
		$render_must = '';
		if ($k[2] == 1) $render_must = '<strong>' . $qm->m['plg_pkwkmail']['must'] . '</strong>';
	...
	if ($attr_err_flag == 1) {
		$result_form .= '<h2>' . $qm->m['plg_pkwkmail']['attr_err_title'] . '</h2>' . "\n";
		$result_form .= '<p>' . $qm->m['plg_pkwkmail']['attr_err_mes'] . '</p>' . "\n";
		$result_form .= '<ul>' . "\n";
	...
	$result_form .= '<input type="submit" value="' . $qm->m['plg_pkwkmail']['button'] . '" name="submit" /></p>' . "\n";
...
function PKWKMAIL_confirm($attr, $a_page)
{
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		foreach ($processed_post as $key => $value) {
			...
			if (strpos($key, '_email') && $attr['attr'][$attr_key_n][2] == 1) {
				...
				if ($rc['err']) {
					$render_err_flag = 1;
					$render_err .= "\t" . '<li><strong>' . sprintf($qm->m['plg_pkwkmail'][$rc['msg']], $err_title) .
						'</strong></li>' . "\n";
				}
			}
			...
			if ($attr['attr'][$attr_key_n][2] == 1) {
				if (is_array($attr['attr'][$attr_key_n][3]) && !$value | $value == '') {
					$render_err .= "\t" . '<li><strong>' . sprintf($qm->m['plg_pkwkmail']['invalid_nosel'], $err_title) . '</strong></li>' . "\n";
					$render_err_flag = 1;
				} elseif (!$value) {
					$render_err .= "\t" . '<li><strong>' . sprintf($qm->m['plg_pkwkmail']['invalid_noval'], $err_title) . '</strong></li>' . "\n";
					$render_err_flag = 1;
				}
			}
			++$attr_key_n;
	...
	if ($render_err_flag == 0) {
		...
		$result_cnfm .= '<input type="button" name="back" id="back" onclick="history.back();" value="' . $qm->m['plg_pkwkmail']['btn_revise'] . '" /> ';
		$result_cnfm .= '<input type="submit" value="' . $qm->m['plg_pkwkmail']['button_send'] . '" name="submit" style="font-weight:bold" /></p>' . "\n";
		$result_cnfm .= '</form>' . "\n";
		...
		$result_cnfm .= '<input type="button" name="back" id="back" onclick="history.back();" value="' . $qm->m['plg_pkwkmail']['btn_back'] . '" /> ';
...
{
	global $vars, $google_apps, $google_apps_domain, $passwd;
	$qm = get_qm();

	// digest check - 新規生成するダイジェストとPOST値比較
	$digest = md5(join('', get_source($vars['page'])));
	$s_digest = htmlspecialchars($digest, ENT_QUOTES);
	if ($vars['digest'] != $s_digest) die_message(PKWKMAIL_addEditLink('Invalid digest.'));
	...
	if ($rc['err']) die_message(PKWKMAIL_addEditLink(sprintf($qm->m['plg_pkwkmail'][$rc['msg']], $mail_content['mail_adrs'])));
	...
	if ($send_err_admin)  $result_finish .= '<p>' . $qm->m['plg_pkwkmail']['send_err_admin'] . '</p>' . "\n";
	if ($send_err_client) $result_finish .= '<p>' . $qm->m['plg_pkwkmail']['send_err'] . '</p>' . "\n";
...
function PKWKMAIL_format_date($val)
{
	...
	$val += ZONETIME;
	$date = gmdate($qm->m['plg_pkwkmail']['fmt_date'], $val) . ' [' . $weeklabels[gmdate('w', $val)] . '] ' . gmdate($qm->m['plg_pkwkmail']['fmt_time'], $val);
...
function PKWKMAIL_addEditLink($str)
{
	global $vars;
	$qm = get_qm();
	$page = $vars['page'];
	$editlink = '&nbsp;&nbsp;<a href="index.php?cmd=edit&page=' . urlencode($page) . '">' . $qm->m['plg_pkwkmail']['link_revise'] . '</a>';
	$str .= $editlink;
	return $str;
}
