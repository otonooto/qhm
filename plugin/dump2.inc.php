...
function plugin_dump2_download()
{
	...
	$filecount = 0;
	$tar = new tarlib();
	$tar->create(CACHEQHM_DIR, $arc_kind) or
		die_message($qm->m['plg_dump2']['err_mktmp_failed']);
...
/////////////////////////////////////////////////
// ファイルのダウンロード
function plugin_dump2_download_full()
{
	...
	if (isset($vars['_p_dump2_memlimit'])) {

		if (is_numeric($vars['_p_dump2_memlimit_value'])) {
			ini_set("memory_limit", $vars['_p_dump2_memlimit_value'] . "M");
		} else {
	...
	foreach ($bk_files as $file) {
		if (file_exists($file))
			$zipFile->addFile(file_get_contents($file), $file);
	}
...
function zip_add_dir($dir, &$zip, $exclude)
{
	...
	if ($dhandle) {
		while (false !== ($fname = readdir($dhandle))) {
			...
			if (is_dir($dir . '/' . $fname) && $fname != '.' && $fname != '..') {
				zip_add_dir("$dir/$fname", $zip, $exclude);
			} else {
				if (!preg_match('/^' . $exclude . '/', $fname) && $fname != '.' && $fname != '..') {
					$handle = fopen($dir . '/' . $fname, "rb");
					$targetFile = fread($handle, filesize($dir . '/' . $fname));
					fclose($handle);
					//				echo './'.$dir.'/'.$fname.'<br />';
					$zip->addFile($targetFile, './' . $dir . '/' . $fname);
				}
			}
		}
		closedir($dhandle);
...
/////////////////////////////////////////////////
// 入力フォームを表示
function plugin_dump2_disp_form()
{
...
<p><label for="_p_dump2_adminpass_dump"><strong>{$qm->m['plg_dump']['label_adminpass']}</strong></label>
  <input type="password" name="pass" id="_p_dump2_adminpass_dump" size="12" />
  <input type="submit"   name="ok"   value="{$qm->m['plg_dump']['btn_download']}" />
</p>
<p style="margin-left:2em;"><input type="checkbox" name="_p_dump2_memlimit"> {$qm->m['plg_dump']['label_memory_limit']} <input type="text" size="4" name="_p_dump2_memlimit_value" value="64" />MB<br />
...
<br />
  <input type="radio" name="pcmd" id="_p_dump2_tgz" value="tgz" checked="checked" />
  <label for="_p_dump2_tgz"> 〜.tar.gz 形式</label><br />
  <input type="radio" name="pcmd" id="_p_dump2_tar" value="tar" />
  <label for="_p_dump2_tar">〜.tar 形式</label>
</p>
<p><strong>{$qm->m['plg_dump']['frm_bk_dirlist']}</strong>
<br />
  <input type="checkbox" name="bk_wiki" id="_p_dump2_d_wiki" checked="checked" />
  <label for="_p_dump2_d_wiki">wiki</label><br />
  <input type="checkbox" name="bk_attach" id="_p_dump2_d_attach" />
  <label for="_p_dump2_d_attach">attach</label><br />
  <input type="checkbox" name="bk_backup" id="_p_dump2_d_backup" />
  <label for="_p_dump2_d_backup">backup</label><br />
</p>
<p><strong>{$qm->m['plg_dump']['frm_bk_option']}</strong>
<br />
  <input type="checkbox" name="namedecode" id="_p_dump2_namedecode" />
  <label for="_p_dump2_namedecode">{$qm->m['plg_dump']['label_namedecode']}</label><br />
</p>
<p><label for="_p_dump2_adminpass_dump"><strong>{$qm->m['plg_dump']['label_adminpass']}</strong></label>
  <input type="password" name="pass" id="_p_dump2_adminpass_dump" size="12" />
  <input type="submit"   name="ok"   value="{$qm->m['plg_dump']['btn_download']}" />
...
EOD;
	...
	if (PLUGIN_DUMP_ALLOW_RESTORE) {
		...
		$data .= <<<EOD
<h3>{$qm->m['plg_dump']['frm_rstr_header']} (*.tar, *.tar.gz)</h3>
...
$frm_rstr_ntc_maxsize<br />
  ...
  <label for="_p_dump2_upload_file">{$qm->m['plg_dump']['label_file']}:</label>
  <input type="file" name="upload_file" id="_p_dump2_upload_file" size="40" />
</p>
<p><label for="_p_dump2_adminpass_restore"><strong>{$qm->m['plg_dump']['label_adminpass']}</strong></label>
  <input type="password" name="pass" id="_p_dump2_adminpass_restore" size="12" />
  <input type="submit"   name="ok"   value="{$qm->m['plg_dump']['btn_restore']}" />
</p>
 </div>
</form>
EOD;
...
class tarlib
{
	...
	function add_dir($dir, $mask, $decode = FALSE)
	{
		...
		if ($dp === FALSE) {
			...
			die_message($qm->replace('plg_dump2.err_tar_not_found', $dir));
		}
		...
		foreach ($files as $name) {
			...
			if ($size === FALSE) {
				@unlink($this->filename);
				die_message($qm->replace('plg_dump2.err_tar_not_found', $name));
			}
